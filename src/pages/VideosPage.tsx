  ...
  return (
    <div className="w-full pt-36 sm:pt-40 pb-24 px-4 sm:px-6 relative">
      <div className="max-w-6xl mx-auto space-y-14">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 text-left">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-secondary text-foreground text-xs font-semibold border border-border">
              ...
              <span>Live Repair Demonstrations · AZTEK CARE</span>
            </div>
            <h1 className="text-4xl sm:text-6xl font-black tracking-tight text-foreground">
              Watch real repairs.
            </h1>
            <p className="text-sm sm:text-base text-muted-foreground">
              See our technicians restore shorted motherboards, cracked OLEDs, and water-damaged circuits under the microscope.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-1.5 p-1 bg-secondary rounded-2xl border border-border/60">
            {categories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => setActiveTab(cat.id)}
                className={`text-xs font-medium px-4 py-2 rounded-xl transition-all cursor-pointer ${
                  activeTab === cat.id
              ...
              >
                <div className="relative aspect-[16/10] overflow-hidden bg-muted">
                  ...
                  <div className="absolute top-3 left-3 z-10">
                    <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold px-2.5 py-1 rounded-md bg-black/60 text-white backdrop-blur-md">
                  ...
                  >
                    <div className="w-11 h-11 rounded-full bg-white/90 text-black flex items-center justify-center shadow-md group-hover:scale-110 transition-transform">
                      <Play className="w-4 h-4 fill-current ml-0.5" />
                    </div>
                  </a>

                  <div className="absolute bottom-3 left-3 z-10 text-[11px] font-medium text-white/90">
                    {post.views}
                  </div>
                </div>
        ...
        {/* Social Profiles Direct Follow Pill */}
        <div className="pt-6 flex flex-wrap items-center justify-center gap-3">
          <a
            href="https://facebook.com"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-secondary hover:bg-secondary/80 text-foreground text-xs font-medium border border-border/70 transition-colors"
          >
          ...
          >
            ...
            <span>Follow Instagram Reels</span>
            <ArrowUpRight className="w-3.5 h-3.5 opacity-50" />
          </a>
          <a
